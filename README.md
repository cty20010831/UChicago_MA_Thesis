...
Afterwards, it is time to switch to R for the final statistical analysis, including descriptive statistics, correaltional analysis, mediation analysis, multilevel regression, and visualization. The R script is located in the `analysis/scripts/` folder. For my own analysis, I used RStudio to run the script and I have `analysis.Rproj` right under the `analysis` folder. You can open the R project file `analysis.Rproj` in RStudio and simply run [main_results.R](analysis/scripts/main_results.R) to reproduce the results. It will save the results under `analysis/main_results/` folder.

### Results
Mood induction successfully altered participants’ self-reported arousal and valence levels, validating the affective manipulation. However, one-way ANOVA results revealed no significant differences in originality or any flexibility measures across mood conditions, failing to support the hypothesized effect of mood on creativity. Given the lack of between-group effects, the planned mediation analysis was not conducted.

Instead, correlational and multilevel regression analyses were employed to investigate the relationship between process-level flexibility measures and originality. These analyses revealed a meaningful dissociation between two modes of cognitive flexibility: persistent exploratory breadth (captured by average entropy and average Bhattacharyya distance) and adaptive switching (captured by inflection proportions and Divergent Semantic Integration). While average-level metrics were uncorrelated with originality, inflection-based and semantic integration measures emerged as robust predictors of originality, both at the correlational and multilevel modeling levels.

Together, these results raised caveats on transient mood manipulations in creativity research, while illuminating the value of process-level, adaptive flexibility as a core mechanism underlying originality. Moreover, it demonstrates the utility of integrating drawing-based tasks with machine learning and NLP to capture the dynamic, cross-modal mechanisms underpinning original thought.
